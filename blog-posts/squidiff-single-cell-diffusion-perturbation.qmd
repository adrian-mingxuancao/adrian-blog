...
Squidiff conditions a reverse diffusion process on a **semantic latent** \(z_{\text{sem}}\) from a **semantic encoder** (described in the paper as VAE-derived). The motivation, borrowed from **diffusion autoencoder** ideas, is that the “low-level” diffusion latent \(x_T\) alone is often **not semantically smooth** for interpolation; adding \(z_{\text{sem}}\) steers sampling toward biologically meaningful directions.
...
At a high level, Squidiff is a **conditional diffusion model** that predicts transcriptome vectors through a reverse diffusion process conditioned on a semantic embedding \(z_{\text{sem}}\).
...
\[
\mathcal{L}_{\text{MSE}}=\sum_{t=1}^T \mathbb{E}_{x_0,\epsilon_t}\left[\|\epsilon_\theta(x_t,t,z_{\text{sem}})-\epsilon_t\|_2^2\right],\quad \epsilon_t\sim\mathcal{N}(0,1).
\]

When the implementation **learns per-timestep variance** (the “improved diffusion” pattern), the objective often adds a **variational bound** term \(\mathcal{L}_{\text{VB}}\) alongside \(\mathcal{L}_{\text{MSE}}\), so the total loss can be written schematically as \(\mathcal{L}=\mathcal{L}_{\text{MSE}}+\mathcal{L}_{\text{VB}}\) when `learn_sigma` or equivalent flags are enabled.

In code, \(x_t\) is sampled with a `q_sample`-style forward diffusion, the model is called on \((x_t, t)\) with access to the baseline \(x_0\) for encoding \(z_{\text{sem}}\), and MSE is computed against the sampled noise \(\epsilon\) when using \(\epsilon\)-prediction; optional VB terms appear when variance is learned rather than fixed.

## Conditioning and generation

**Conceptually**, the model learns perturbation or developmental “direction” in semantic space and uses that direction to generate intermediate or perturbed transcriptomes. For differentiation from day 0 to day 3, the paper describes extracting semantic latents \(z^{(1)}_{\text{sem}}, z^{(2)}_{\text{sem}}\) and using \(\Delta z_{\text{sem}}\) as a direction vector.
...
\[
z_{\text{interp}}=\text{mean}(z_{\text{origin}})+(\text{direction})\cdot \text{scale},
\]

optionally adding noise for **per-cell diversity**, then generating expression with DDIM conditioned on \(z_{\text{interp}}\).

```{mermaid}
flowchart LR
  A[Input: baseline expression x0] --> B[Preprocess: log-normalize, HVGs]
  B --> C[Semantic encoder: z_sem]
  D[Metadata: cell type, time, perturbation, dose] --> C
  C --> E[Edit latent: delta z or interpolation]
  E --> F[Initialize x_T ~ Normal]
  F --> G[Denoiser: epsilon_theta of x_t, t, z]
  G --> H[DDIM steps to x0_hat]
  H --> I[Predicted transcriptome]
```
...
3. **Ablate semantic conditioning**: remove or shuffle \(z_{\text{sem}}\) and check whether developmental or perturbation **interpolation collapses**—this tests the core diffusion-autoencoder motivation.
...
- **Hybrid with foundation models**: use a **pretrained single-cell embedding model** to produce \(z_{\text{sem}}\), keeping Squidiff (or a lighter denoiser) as the conditional generator on top of frozen or fine-tuned representations.
...
```{mermaid}
timeline
  title Single-cell perturbation modeling (schematic)
  2019 : scGen
  2020 : trVAE
  2021 : CPA
  2023 : CellOT
  ...
  2024 : GEARS
  2025 : Squidiff
  2026 : Emerging diffusion-style perturbation models
```
...
sequenceDiagram
  ...
  participant Z as z_sem
  participant Sam as DDIM sampler
  participant Den as Denoiser
  participant Out as Predicted x0

  X0->>Enc: encode with metadata
  Enc-->>Z: z_base
  Note over Z: z_target = edit z_base
  Sam->>Sam: x_T ~ Normal(0,I)
  loop t = T to 1
    Sam->>Den: epsilon_hat = model(x_t, t, z_target)
    Den-->>Sam: epsilon_hat
    Sam->>Sam: DDIM update x_{t-1}
  end
  Sam-->>Out: x0_hat
```
